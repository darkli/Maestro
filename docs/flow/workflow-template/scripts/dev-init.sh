...
RESULT_DESIGN_SOURCE=""
RESULT_STAGES_PRELOADED=""
...
# (d2) 设计上游产物检测
# ============================================================
detect_design_upstream() {
  # 仅 feature 和 bugfix 类型需要检测设计上游（design 类型本身不需要）
  case "$ARG_TYPE" in
    feature|bugfix) ;;
    *) verbose "类型 $ARG_TYPE 无需检测设计上游"; return 0 ;;
  esac

  # 扫描 design-*-$ARG_NAME/ 目录
  local candidates=""
  local count=0
  for entry in "${WS_BASE}"/design-*-"${ARG_NAME}"/; do
    [ -d "$entry" ] || continue
...
"
  ...
  if [ "$count" -eq 0 ]; then
    verbose "未找到匹配的 design workspace: design-*-${ARG_NAME}"
    return 0
  fi

  # 多个匹配时取日期最新的（目录名格式 design-YYYYMMDD-name，按名称排序取最后一个）
  local latest_dir
  latest_dir=$(printf '%s' "$candidates" | sort | tail -1)
  # 去掉末尾换行和斜杠
  latest_dir=$(printf '%s' "$latest_dir" | sed 's|/$||')

  if [ ! -d "$latest_dir" ]; then
    verbose "设计上游目录不存在: $latest_dir"
    return 0
  fi

  local design_dir_name
  design_dir_name=$(basename "$latest_dir")
  local design_rel_path=".claude/workspace/${design_dir_name}"

  verbose "找到设计上游: $design_rel_path"

  # 检查目标 design workspace 中的文件
  local has_01=false
  local has_02=false
  local stages_list=""

  if [ -f "${latest_dir}/01-requirements.md" ]; then
    has_01=true
    stages_list="1"
  fi

  if [ -f "${latest_dir}/02-design.md" ]; then
    has_02=true
    if [ -n "$stages_list" ]; then
      stages_list="${stages_list}, 2"
    else
      stages_list="2"
    fi
  fi

  if [ "$has_01" = false ] && [ "$has_02" = false ]; then
    verbose "设计上游目录存在但无 01/02 文件"
    return 0
  fi

  # 复制文件到新创建的 workspace（非 symlink）
  local ws_abs="${PROJECT_ROOT}/${RESULT_WORKSPACE}"
  local source_tag="<!-- 来源: ${design_dir_name} -->"

  if [ "$ARG_DRY_RUN" = true ]; then
    verbose "[DRY-RUN] 将从 $design_rel_path 复制 01/02 文件"
  else
    if [ "$has_01" = true ]; then
      { printf '%s\n' "$source_tag"; cat "${latest_dir}/01-requirements.md"; } > "${ws_abs}/01-requirements.md"
      verbose "已复制 01-requirements.md（来源: ${design_dir_name}）"
    fi
    if [ "$has_02" = true ]; then
      { printf '%s\n' "$source_tag"; cat "${latest_dir}/02-design.md"; } > "${ws_abs}/02-design.md"
      verbose "已复制 02-design.md（来源: ${design_dir_name}）"
    fi
  fi

  RESULT_DESIGN_SOURCE="$design_rel_path"
  RESULT_STAGES_PRELOADED="$stages_list"
  verbose "设计上游检测完成: source=$RESULT_DESIGN_SOURCE stages=$RESULT_STAGES_PRELOADED"
}
...
check_progress_file() {
  ...
  local matched_file=""
  local pf
  for pf in "${WS_BASE}"/_progress-*.md; do
    [ -f "$pf" ] || continue
    has_progress=true
...
emit_result_json() {
  ...
  # design_source：有值时为带引号字符串，否则为 JSON null
  local design_source_val="null"
  if [ -n "$RESULT_DESIGN_SOURCE" ]; then
    design_source_val="\"${RESULT_DESIGN_SOURCE}\""
  fi

  # stages_preloaded 转为 JSON 数组
  local stages_json="[]"
  if [ -n "$RESULT_STAGES_PRELOADED" ]; then
    stages_json="[${RESULT_STAGES_PRELOADED}]"
  fi
  ...
  output_json "{\"workspace\": \"${RESULT_WORKSPACE}\", \"files_created\": ${files_json}, \"upstream_detected\": ${upstream_detected}, \"available_upstreams\": ${upstreams_json}, \"design_source\": ${design_source_val}, \"stages_preloaded\": ${stages_json}, \"progress_updated\": ${progress_val}, \"capabilities\": ${caps_val}, \"warnings\": ${warnings_json}}"
}
