...
本文档定义工作流系统（Skills）对 `CLAUDE.md` 的依赖关系。新项目移植时，对照本契约逐条填写 `CLAUDE.md`，确保工作流系统可以正确运行。

---

## 必需部分（8 项用户填写 + 1 项自动生成）

工作流 Skill 在运行时**必须**能从 `CLAUDE.md` 中找到以下信息。缺失任何一项将导致 Skill 产出质量下降或逻辑错误。

---

### 1. Project Overview（项目概述）

**用途**：让 Claude 了解项目的业务背景和技术全貌，避免产出与项目性质不符的方案。
...
**用途**：Skill 验证阶段执行构建检查和测试时直接引用这些命令。
...
**用途**：Claude 在 /f-dev 和 /f-test 中编写测试时遵循此规范；编码阶段确认测试环境配置。
...
**用途**：Claude 在 /f-design 和 /f-dev 设计阶段参考现有架构；确认文件应放在哪个目录；所有 Skill 了解前后端分离边界和通信方式。
...
**用途**：Claude 在编码阶段遵循此规范；自查阶段以此为准绳。
...
**用途**：Claude 在 /f-dev 编码阶段遵循此顺序，确保不遗漏步骤（如忘记更新 locale 文件）。
...
**用途**：Claude 在阶段结束时执行 `git commit`，提交消息格式从此处读取。
...
**用途**：Claude 在所有 Skill 中输出文档、注释、Workspace 文件时遵循此规范。
...
**用途**：`/f-init` 在生成定制化文件时，读取此节的标签值来裁剪模板中的 `IF:xxx` 条件块。各 Skill 在运行时也引用此节判断哪些检查项适用。
...
**适用场景**：前端包含多语言支持时必须填写，否则 Claude 不知道需要更新哪些文件。

**用途**：Claude 在编码阶段实现包含 UI 文本的功能时，知道需要更新哪些 locale 文件；验证阶段执行 i18n 完整性检查。
...
**适用场景**：项目有自定义设计 Token（Tailwind 或 CSS Variables）时填写，避免使用硬编码颜色值。

**用途**：Claude 编写 UI 组件时使用正确的 Token 名称；自查阶段检查是否有硬编码颜色。
...
**用途**：Claude 编写数据库查询时遵循安全规范；设计数据模型时了解数据库类型。
...
**用途**：Claude 修改 Agent 代码时知道需要版本号和重新编译；验证阶段了解完整的构建流程。
...
**用途**：Claude 了解本地运行时需要哪些环境变量；文档 Skill 在用户指南中提示配置步骤。
...
工作流系统的 Skill 名称（`f-dev`、`f-bugfix` 等）和 Workspace 文件名（`01-requirements.md` 等）是**系统固定标识符**，在 `CLAUDE.md` 和其他配置中**不应引用自定义名称**。这些名称在所有项目中保持一致，确保跨项目的统一执行方式。
...
- **前端框架**：`Capabilities.frontend`。如值非 `false`，自查阶段将执行对应的前端最佳实践检查。
- **WebSocket**：从 Architecture 通信路径表推断的隐式派生标签（仅在内存中，不写入 Capabilities）。如包含 WebSocket 通信路径，编码阶段的自检清单将包含 WebSocket 事件格式检查。
- **静态类型语言**：`Capabilities.static-types`。如值非 `false`，验证阶段将执行类型检查步骤。

### 自动生成部分（无需用户填写）

以下段落由模板自动提供，用户无需编写：

- **Git Safety**：`git-guard.sh` Hook 的行为说明，告知 Claude 所有 git 写操作需先获得用户确认。此段落随模板安装，确保 Claude 理解 Hook 拦截行为。

### 功能验证

- [ ] 运行 `/f-dev 创建一个测试功能`，确认 Claude 能正确读取 CLAUDE.md
- [ ] 检查编码阶段输出的代码是否符合项目代码风格
- [ ] 检查验证阶段是否能成功执行构建和测试命令
