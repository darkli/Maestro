...
在 CLAUDE.md 中指定的类型定义文件中添加共享类型，按照设计文档中的数据结构定义接口和 DTO。
<!-- ENDIF:static-types -->

<!-- IF:backend-api -->
### 第三步：实现后端

按照项目分层架构实现，参考 CLAUDE.md 中 Architecture 部分的分层说明。遵循路由层 → 服务层 → 数据层的模式，所有查询参数化，所有路由添加认证中间件和错误处理。
<!-- ENDIF:backend-api -->

<!-- IF:frontend -->
### 第四步：实现前端

在 CLAUDE.md 中指定的 API 调用模块中追加新函数，创建或修改组件。使用项目 Design Token，所有文本通过国际化函数处理。参考 CLAUDE.md 中 Code Style 部分的规范。
<!-- ENDIF:frontend -->
...
<!-- ENDIF:NOT:backend-api -->
<!-- ENDIF:NOT:frontend -->

<!-- IF:i18n -->
### 第五步：i18n 国际化

每次涉及 UI 文本变更时，必须更新 CLAUDE.md 中指定的所有 locale 文件。翻译键使用 CLAUDE.md 中约定的命名格式。

<!-- PROJECT-SPECIFIC: i18n 文件列表 -->
```
locales/
├── en-US.json    # 英语（基准）
├── zh-CN.json    # 简体中文
...
**i18n 检查清单**：
- [ ] 所有 UI 文本使用国际化函数，无硬编码字符串
- [ ] CLAUDE.md 中指定的所有 locale 文件已更新
- [ ] 翻译键符合项目约定的命名格式
- [ ] 基准语言作为模板，其他语言翻译准确
<!-- ENDIF:i18n -->
...
根据 CLAUDE.md 中 Build & Development Commands 部分的交叉编译命令进行验证：

<!-- PROJECT-SPECIFIC: 交叉编译命令 -->
```bash
# 示例：嵌入式项目
make clean && make all
# 检查产物
...
<!-- IF:i18n -->
- [ ] 无硬编码的 UI 字符串（全部使用国际化函数）
<!-- ENDIF:i18n -->

<!-- IF:design-system -->
- [ ] 使用项目 Design Token（非自定义颜色值）
- [ ] 主题切换样式已适配
<!-- ENDIF:design-system -->
