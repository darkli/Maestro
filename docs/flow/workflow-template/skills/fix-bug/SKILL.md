...
使用与 feature-dev 相同的文件命名规范，但 Workspace 前缀为 `bugfix-`，内容适配 Bug 修复场景。
...
**文件名与 feature-dev 一致**，下游子代理（code-reviewer、integration-validator）无需适配，直接复用。
...
```bash
WS=".claude/workspace/bugfix-$(date +%Y%m%d)-$BUG_NAME"
mkdir -p "$WS"
```

### 步骤 2：记录 Bug 信息

将 Bug 描述写入 `$WS/00-input.md`，包含：问题名称、报告时间、问题描述、错误日志/堆栈跟踪、复现步骤（如提供）、预期行为、实际行为。
