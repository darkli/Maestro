...
本 Skill 提供 **4 阶段精简流水线**，将 f-dev 的 7 个阶段合并为 4 个，减少子代理调用次数。适合小功能、UI 增强、简单修改等场景。

**与 f-dev 的核心区别**：
- 需求分析 + 系统设计合并为 1 个阶段
- 编码 + 测试合并为 1 个阶段（边写代码边写测试，非 TDD）
- 不生成文档（无阶段 7）
- 子代理调用：4 次（vs f-dev 的 7 次）

## Workspace 工作记忆

与 f-dev 使用相同的 Workspace 机制和文件命名。
...
> 如果检测到上游设计产物（来自 `/f-design`）且 01+02 均已预加载并被用户确认，阶段 1 将被跳过。
...
- **确认通过** → 进入阶段 2
- **要求修改** → 重新分析设计
- **升级到完整模式** → 01 和 02 已创建，切换到 `/f-dev` 从阶段 3 继续
...
**当 `stages_preloaded` 同时包含 1 和 2 时**（01 和 02 均已预加载）：

`f-light-dev` 的阶段 1 是"分析与设计"（一个阶段同时输出 01+02）。两个文件都预加载时，阶段 1 整体可跳过。

1. 读取已复制到 `$WS` 中的 `01-requirements.md` 和 `02-design.md` 的核心摘要
2. 向用户展示：
   - 来源 workspace 路径（`design_source` 值）
   - 预加载的内容摘要（功能概述、架构要点、关键设计决策）
3. 询问用户（AskUserQuestion，3 选项）：
   - **使用已有设计，跳过阶段 1**（推荐）→ 直接进入阶段 2（编码与测试）
...
**当 `stages_preloaded` 仅包含 1**（只有 01 预加载，02 缺失）：阶段 1 仍需执行（设计部分缺失），但子代理可参考已有的 01 文件。
...
git commit -m "feat($FEATURE_NAME): 完成编码（轻量）"
```
<!-- ENDIF:NOT:testing -->
...
**子代理输出** → 写入 `$WS/05-review.md`（格式与 f-dev 相同）
...
**执行步骤**：与 f-dev 阶段 6 完全相同。

**子代理输出** → 写入 `$WS/06-validation.md`（格式与 f-dev 相同）
...
1. 保留已创建的 01 和 02
2. 提示用户执行：`/f-dev 从阶段 3 继续，Workspace: $WS`
