...
<!-- 本文件基于 feature-dev 内联，修改流程时请同步 feature-dev/SKILL.md -->

# 快速功能开发 Skill

## 概述

本 Skill 执行与 `/feature-dev` 完全相同的 **7 阶段流水线**，唯一区别是只在 **2 个关键节点** 暂停确认，其余阶段自动流转。适合中等规模功能，用户信任流程、希望减少交互次数。
...
<!-- PROJECT-SPECIFIC: 步骤 3 中的具体文件路径由项目 CLAUDE.md Architecture 部分定义 -->
...
- 读取 CLAUDE.md 中 Architecture 部分引用的核心文件（如类型定义、API 调用模块）
