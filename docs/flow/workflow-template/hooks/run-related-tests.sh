...
CAPABILITY_TESTING="vitest"                    # 值为框架名或 "false"
TEST_CMD="npm test -- --findRelatedTests"      # 项目专属测试命令
SOURCE_PATTERNS="src/*.ts|src/*.tsx"            # 源码文件 glob 模式
TEST_FILE_PATTERNS="src/*.test.ts|src/*.test.tsx" # 测试文件 glob 模式（跳过用）
# === 配置区域结束 ===
