...
- [ ] **功能点 6：System Prompt 外置**
  - 描述：将 `manager_agent.py` 中的 `DEFAULT_SYSTEM_PROMPT` 常量抽取到外部文件 `prompts/system.md`（或 `.txt`）。`ManagerConfig` 新增 `system_prompt_file` 字段指定文件路径。加载优先级：`system_prompt_file` 指定的文件 > `config.yaml` 中的 `system_prompt` 字符串 > 代码内置默认值。
  - 验收标准：修改外部 prompt 文件后，不重启服务，新创建的任务使用新 prompt。

- [ ] **功能点 7：Action 协议定义外置**
  - 描述：将 action 枚举（execute/done/blocked/ask_user/retry）及其在 system prompt 中的格式示例定义移到外部配置。可以作为 system prompt 文件的一部分（模板变量注入），也可以独立文件。
  - 验收标准：新增 action 类型时只需修改外部文件和对应的代码处理逻辑，无需修改 prompt 硬编码。

- [ ] **功能点 8：决策策略外置**
  - 描述：将 system prompt 中「决策原则」部分抽取，支持按场景切换不同决策风格（如"保守型"vs"激进型"）。可通过 config.yaml 中的 `manager.decision_style` 字段选择，或直接在 prompt 文件中自定义。
  - 验收标准：通过修改配置可切换决策风格，无需改代码。

- [ ] **功能点 9：standalone_chat / free_chat 的 system prompt 外置**
  - 描述：将 `standalone_chat` 和 `free_chat` 方法中硬编码的 system prompt 字符串也抽取到外部文件或配置中。
...
- [ ] **功能点 12：config.example.yaml 同步更新**
  - 描述：在 `config.example.yaml` 的 `manager` 段新增 `system_prompt_file` 和 `decision_style` 字段的说明和示例。
...
- **技术约束**：
  ...
  - 项目无 static-types（无 mypy），无测试框架（暂不需要写测试）。
  - 所有用户可见字符串、注释使用中文。
...
- [ ] `bash deploy.sh init` 在全新 VPS 完成完整部署
- [ ] `bash deploy.sh update` 仅传输代码并更新 Python 包
- [ ] `bash deploy.sh update` 完成后自动重启 maestro-daemon（如已启用）
- [ ] `deploy.sh update` 在未部署环境上友好报错
- [ ] `deploy.sh`（无参数）进入交互菜单，菜单含 4 项（首次部署、业务更新、查看状态、清理卸载）
- [ ] `deploy.sh foo`（无效参数）打印 usage 并退出
- [ ] 所有原有功能不受影响

### 方案 3

- [ ] 配置 `system_prompt_file` 后，ManagerAgent 从文件加载 prompt
- [ ] 修改 prompt 文件后，下次 `decide()` 调用自动使用新内容（热加载）
- [ ] prompt 文件不存在时自动生成默认内容
- [ ] 不配置 `system_prompt_file` 时行为与当前完全一致（向后兼容）
- [ ] `standalone_chat` / `free_chat` 的 prompt 也支持外置
- [ ] `config.example.yaml` 包含新配置项的说明
- [ ] `prompts/` 目录包含所有默认 prompt 文件

## 风险评估

- **风险 1：deploy.sh 重构引入 regression** → 应对策略：保持 `do_deploy()` 函数内部逻辑不变，仅拆分调用入口。`init` 直接调用 `do_deploy()`，`update` 调用新函数 `do_update()`。菜单选项映射到同一组函数。
